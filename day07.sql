...
SELECT to_char(12345, '9999') 
  FROM dual
;
SELECT to_char(12345, '99999') AS "12345" -- 왼쪽 정렬
  FROM dual
;
SELECT 12345 -- 오른쪽 정렬
  FROM dual
;
-- 숫자가 문자회 되어 출력되면 왼쪼 정렬로 바뀐다.
...
SELECT to_char(12345,'99999999.99') as 데이터
  FROM dual
;
-- 숫자 패턴에서 3자리씩 끊어 읽기, 소수저 표현
SELECT to_char(12345,'9,999,999.99') as 데이터
  FROM dual
;

----- 2. to)date() : 날짜 패턴에 맞는 문자 값을 날짜 연산이 가능한 날짜 타입으로 변경
SELECT to_date('2019-05-25', 'YYYY-MM-DD') "today"
     , '2019-05-28' "today(문자)"
  FROM dual
;
/*
...
SELECT to_date('2019-5월-25', 'YYYY-MON-DD') "today"
     , '2019-5월-28' "today(문자)"
  FROM dual
;

-- 10일 후의 날짜 연산
SELECT to_date('2019-5월-25', 'YYYY-MON-DD') + 10 "today+ 10일"
  FROM dual
;
-- today +10 일
-- 19/06/07
...
SELECT '2019-5월-25' + 10 "today+ 10일"
  FROM dual
;
/*
...
-- 지급 비율이 다음과 같다고 할때, 각 직원들의 경조사비 지원금을 구해보자.
-- 사번, 이름, 직무와 함께 구해보자
/*
 CLERK      : 5%
 SALESMAN   : 4%
